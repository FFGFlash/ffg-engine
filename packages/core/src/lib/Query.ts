  ...
  >(...names: N): Query<A> {
    ...
    for (const name of names) {
      if (this.accesses.has(name))
        throw new Error(`Component "${name}" is already included in the query.`)
  ...
  ? Head extends [infer Name extends ComponentName, infer Mode extends AccessMode]
    ? Mode extends false
      ...
      ? AccessListToComponentData<
        ...
        >
      : AccessListToComponentData<Tail extends AccessList ? Tail : [], [...A, ComponentType<Name>]>
    : never
  : A
