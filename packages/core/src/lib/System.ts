...
export class System<Args extends readonly unknown[] = []> {
  /** @internal */
  _getDeps: ((data: World['data']) => readonly any[]) | undefined;
  /** @internal */
  _fn: ((...args: any[]) => void | Promise<void>) | undefined;
  /** @internal */
  ...
  deps<A extends readonly unknown[]>(
    deps: (data: World['data']) => Readonly<A>
  ): System<A> {
    if (this._getDeps)
      throw new Error('Dependencies already set for this system.');
    this._getDeps = deps;
  ...
  before(...systems: string[]): this {
    ...
    for (const system of systems) {
      if (this._after?.has(system))
        throw new Error(
          `System ${this._name} cannot be both before and after ${system}.`
        );
      if (this._before.has(system))
        throw new Error(
          `System ${system} is already set as a before dependency for ${this._name}.`
        );
  ...
  after(...systems: string[]): this {
    ...
    for (const system of systems) {
      ...
      if (this._after.has(system))
        throw new Error(
          `System ${system} is already set as an after dependency for ${this._name}.`
        );
