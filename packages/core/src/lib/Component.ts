...
export type ComponentType<T extends ComponentName = ComponentName> = ComponentRegistry[T]
...
export class ComponentMap extends Map<ComponentName, number> {
  ...
  register(name: ComponentName): number {
    if (this.has(name)) throw new Error(`Component "${name}" is already registered.`)
    const id = this.size
    super.set(name, id)
    return id
  }
